...
This threat model applies to the implemented stock Chrome/Edge Silo, local proxy/Mihomo path, Native Messaging Host, optional Companion, V0.7 adapter/package control plane, V0.8 local-environment controllers, and V0.9 desktop/Remote-Agent control plane. It distinguishes executable control planes, the internally CMS-signed Formal-v3 package path, and external acceptance inputs: a package interface, build check, signer pin, legal VM image, real remote browser Provider, media stream, or real Windows/WAN result does not by itself prove current product acceptance.
...
- Trusting an unsigned/unpinned third-party controlled-engine package, unknown VM image, or arbitrary remote Provider. The production adapter accepts only reviewed, fixed package inputs; current product availability still depends on direct runtime evidence and the applicable environment gate.
...
- A controlled-engine package and its signer are separate supply-chain trust boundaries. Production accepts only the frozen manifest schema, canonical files, exact hashes and a pinned signing certificate; a build with an empty production signer set stays fail-closed.
...
| ------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------- |
| Manifest traversal, package substitution or downgrade   | Strict schema, canonical package root, exact executable/file hashes, CMS detached signature, signer pin, version lock, update receipt, rollback and emergency disable | rc2 exact pristine Windows Sandbox lifecycle accepted; strict non-admin behavior, broader Windows promotion, pin rotation/revocation, and public release remain separate gates |
...
V0.7–V0.9 source and state-machine tests reduce implementation risk; they do not convert package verification, build checks, or an interface into current user-journey runtime evidence. The final completion audit must continue to separate implementation, qualification, product acceptance, and unavailable capability claims.
