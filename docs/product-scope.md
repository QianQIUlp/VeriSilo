...
Standard remains a permanent baseline. The Managed Identity execution path is now implemented in the product: Camoufox M0–M2-W, M3-0, FP1–FP4, and clean M3-WI Attempt 4 are closed at their documented evidence layers; the production `ExternalPackageEngineAdapter`, Formal-v3 package build and internal detached CMS signing, Desktop signer pin, Managed Silo creation/configuration/network/run path, and current-user NSIS packaging are implemented. Current work is **Pre-RC product stabilization**: real product QA, targeted fixes, integration, baseline advancement, and fresh QA. There is no current-source RC; the local `v0.1.0-rc1` artifact is a historical, superseded candidate whose runtime acceptance was never run. Local virtual and self-hosted remote environments remain later, optional upgrades rather than substitutes for Profile or identity semantics. See the [identity platform north star](identity-platform-north-star.md), [Camoufox engine decision](camoufox-managed-engine-decision.md), [current Camoufox status](camoufox-program-status.md), and [environment roadmap](environment-roadmap.md).
