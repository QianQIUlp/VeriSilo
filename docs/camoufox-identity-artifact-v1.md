# VeriSilo M1.1 + M2-0 + M2.0.1 + M2.0.2 — Resolved Camoufox Identity Artifact

Status: **M2.0.2 accepted on this host — observations only,
...
| 正常生命周期与进程树清理 | 通过 |
| 异常进程无法退出时 fail-closed | **通过（quarantine + 锁保留 + 持久记录 + PID/start-time 校验）** |
...
Accepted run：**`run-1785848308-3bce2bb1`**
（report.sha256 `8ba4d9c2…887fe`）。
...
Accepted run：**`run-1785848328-b9bec18c`**。
...
Accepted run：**`run-1785848341-c249809a`**。digest / missing-field /
