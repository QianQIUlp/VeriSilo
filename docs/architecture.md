...
The current `main` branch contains the stock launcher, the external-package `EngineAdapter` control contract, and the standalone Python Camoufox Host plus accepted Linux/native-Windows M0–M2-W evidence merged by [PR #10](https://github.com/QianQIUlp/VeriSilo/pull/10). The two execution paths are still separate: the Host is not a signed desktop engine package, Tauri does not launch it, and it is not a shipped capability. [M3-0](camoufox-m3-engine-adapter-task.md) now freezes the package entrypoint, dedicated Host transport, Artifact binding, lifecycle, and honest capability/evidence mapping before a separate native Windows desktop Gate. Current checkpoints are maintained in [the Camoufox program status](camoufox-program-status.md).
