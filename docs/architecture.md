...
Tauri desktop UI
  ...
  │    └─ verified external controlled-engine adapter (Formal-v3 package path)
  │         └─ Camoufox Host/package, Artifact binding, and production launch seam
  │              └─ clean M3-WI Attempt 4: historical test-only qualification
  ├─ network coordinator
...
The current source contains the stock launcher, the external-package `EngineAdapter` contract and production implementation, the standalone Python Camoufox Host, accepted Linux/native-Windows M0–M2-W evidence, FP1–FP4 qualification, and the clean M3-WI Attempt 4 test-only qualification. M3-0 accepted the package entrypoint, dedicated Host transport, Artifact binding, lifecycle, and honest capability/evidence contract at `e96ef3f`; subsequent work added the Formal-v3 package builder, internal detached CMS signing, Desktop signer pin, Managed Silo product path, and current-user NSIS packaging. The current-source `v0.1.0-rc2` package and installed lifecycle passed in the exact pristine Windows Sandbox acceptance environment; strict non-admin semantics, broader Windows compatibility, outer Authenticode signing, and public release remain separate boundaries. Current routing is maintained in [the Camoufox program status](camoufox-program-status.md).

FP1 deterministic Artifact projection, FP2 cross-realm consistency/replay, FP3 network/geo/timezone/locale coherence, FP4 bounded ordinary-site compatibility, and clean M3-WI Attempt 4 are the completed historical qualification chain for the current Managed Engine. They do not create an FP5; the current-source rc2 installed acceptance is a separate, bounded product evidence layer. Current checkpoints and next tasks are maintained in [the Camoufox program status](camoufox-program-status.md).
...
The optional adapters are stronger environment backends, not claims about the stock launcher. V0.7 now has a versioned production adapter, package-signature/update/rollback state machine, per-Silo configuration, short-token bootstrap path, and a signed Formal-v3 package path; its current-source rc2 package/runtime and exact Windows Sandbox installed acceptance are recorded separately from the remaining strict non-admin, broader Windows, and public-release boundaries. V0.8 has executable backend controllers and fixed scripts, while real WSL/Sandbox/Hyper-V browser evidence still requires compatible Windows hosts and legal guest images. V0.9 has a real desktop control plane and self-hosted TLS Agent, while the actual VM/browser/media Provider remains an external implementation gate. Their exact evidence states and release requirements live in [the environment roadmap](environment-roadmap.md). Stock Chrome/Edge remains a supported baseline after additional engines arrive.
