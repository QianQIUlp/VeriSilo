...
The current source contains the stock launcher, the external-package `EngineAdapter` contract and production implementation, the standalone Python Camoufox Host, accepted Linux/native-Windows M0–M2-W evidence, FP1–FP4 qualification, and the clean M3-WI Attempt 4 test-only qualification. M3-0 accepted the package entrypoint, dedicated Host transport, Artifact binding, lifecycle, and honest capability/evidence contract at `e96ef3f`; subsequent work added the Formal-v3 package builder, internal detached CMS signing, Desktop signer pin, Managed Silo product path, and current-user NSIS packaging. The production path is implemented and release checks are closed; clean Windows 11 product acceptance is still a separate release-readiness boundary. Current routing is maintained in [the Camoufox program status](camoufox-program-status.md).

FP1 deterministic Artifact projection, FP2 cross-realm consistency/replay, FP3 network/geo/timezone/locale coherence, FP4 bounded ordinary-site compatibility, and clean M3-WI Attempt 4 are the completed historical qualification chain for the current Managed Engine. They do not replace the current pre-RC product QA loop or create an FP5. Current checkpoints and next tasks are maintained in [the Camoufox program status](camoufox-program-status.md).
