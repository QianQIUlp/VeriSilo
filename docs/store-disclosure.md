...
- Network verification never runs automatically. After a confirmation and optional-host permission grant, it contacts `ipwho.is` for the visible exit IP/geolocation/ASN and Cloudflare 1.1.1.1 plus Google Public DNS for a fixed `example.com` DoH comparison. Those providers receive the user's request IP; the result is kept only in trusted session storage and can be cleared from the UI.
...
- The public-DNS comparison does not inspect the browser or operating system resolver and is not presented as proof that local DNS is free from interception. IP reputation and blacklist status are not scored in this version.
