...
| 改口令、加密备份/恢复、版本迁移                                         | **已实现且自动验证**           | 新 Argon2id salt/KEK 重包同一 DEK；schema 1–6 真实加密 fixture 逐版解锁/恢复迁移到 7；原子重写/稳定重开；schema 7 orphan receipt 改口令、备份、恢复；未知/降级/篡改/错误口令拒绝                                                                                                                                       | Windows 磁盘故障、权限和跨卷路径 E2E；备份 UI 人工流程                                                                       | 备份有意不包含浏览器 Profile；恢复不自动删除旧孤儿目录。                                                                        |
