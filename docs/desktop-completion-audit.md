...
- Camoufox standalone Host、v3 Artifact 与 Linux/Windows M0–M2-W accepted evidence 已通过 PR #10 合入 `main`，但仍没有接入桌面 EngineAdapter/Tauri，也不是签名发布包；进入主线不能称为 shipped Managed Identity 能力。
...
| Camoufox standalone 原型                                                           | **已实现仅实验室验证** | `main` 已有固定 Linux/Windows 资产、v3 Artifact、standalone Host 与 M0–M2-W evidence manifest；见 [`camoufox-program-status.md`](camoufox-program-status.md) | M3-0 contract 集成后再做原生 Windows M3-WI                                                          | 未接 Tauri、未成为签名发布包；standalone evidence 不能替代桌面 E2E。    |
...
| apply/verify/restore、站点回退、包更新/回滚/禁用                                   | **部分实现**           | `main` 有 EngineAdapter control/receipt 合约和独立 Host launch/status/close，但两者未连接                                                                    | 按 [`M3-0`](camoufox-m3-engine-adapter-task.md) 建立专用 transport；真实 package/fallback 另设 Gate | 不能把两套各自存在的协议称为已经集成，也不能伪造 generic receipt。      |
| Canvas/WebGL/字体/UA/UA-CH/语言/时区/Window/iframe/Worker/请求头/TLS/QUIC 直接证据 | **部分实现**           | Linux/Windows probe 已证明指定 ObservedWebsiteDigest 稳定与身份分离，并如实排除 Canvas/宿主字体                                                              | 跨上下文、真实 ClientHello/QUIC、managed fonts 和逐字段 capability evidence                         | `verified=false`；TLS/QUIC、字体隔离、Canvas 身份与不可检测均不得宣称。 |
...
- 不能声称 V0.7–V0.9 已作为完整产品交付；`main` 中彼此尚未连接的 EngineAdapter、standalone Camoufox Host、accepted Windows evidence 与真实签名发布验收必须分别表述。
