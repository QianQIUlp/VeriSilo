...
RECIPE_REL = Path("apps/camoufox-host/build/r1-formal-v3")
...
EXPECTED_COMPLETE_PATCH_PATHS = [
    ...
    "apps/camoufox-host/patches/camoufox/v152.0.4-beta.28-r1/0007-verisilo-ff152-search-schema-repair.patch",
]
EXPECTED_PATCH_BINDINGS = {
    ...
    "0007": ("3902cc7187362a306954eb7b18cedb06f74c454d26cc543c28c1fef069a054bb", 752),
}
...
def _validate_recipe_and_mode(lock: dict) -> None:
    ...
    if recipe.get("name") != "camoufox-152.0.4-beta.28-r1-formal-v3":
...
def _validate_patch_contract(lock: dict) -> None:
    ...
    if incremental is not None and (
        ...
        or [item.get("id") for item in incremental]
        != ["0003", "0003a", "0004", "0005", "0006", "0007"]
    ...
    search_schema = [item for item in seams if item.get("id") == "0007"]
    if search_schema != [
        {
            "id": "0007",
        ...
        raise BuildFailure("Formal R1 0007 seam binding is not exact")
