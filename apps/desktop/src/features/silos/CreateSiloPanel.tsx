...
export function CreateSiloPanel({
  ...
  const [setupOpen, setSetupOpen] = useState(Boolean(managedTemplate));
  const [setupStarted, setSetupStarted] = useState(Boolean(managedTemplate));
  ...
  return (
    <>
      <section className="create-hero panel">
        <IdentityMark id="new-identity" color="#1553ff" />
      ...
      >
        ...
        >
          <button
            ...
            type="button"
          >
            <IdentityMark id="standard-mode" color="#1553ff" />
            <strong>系统浏览器</strong>
          ...
          <button
            ...
            type="button"
          >
            <IdentityMark id="managed-mode" color="#cc4c25" />
      ...
      <div className="composer-definition" hidden={!setupOpen}>
        {!setupStarted ? null : creationMode === "managed" ? (
          <ManagedSiloForm
            busy={busy}
            color={color}
            initialColor={color}
              ...
              <fieldset disabled={busy || mihomoBusy} hidden={!advancedOpen}>
                ...
                <div className="proxy-import-card">
                  <div>
                    ...
                    <span>
                      支持 host:port、host:port:user:password 或带协议的代理
                      URL， 会自动填入对应的网络方式。
                    </span>
                  </div>
